...
class LinearDict():
    ...
    def __init__(self, iterable=[]):
        self.d = 1
        self.t = [None for i in range(2**self.d)]
        self.q = 0
        self.n = 0
        self.dl = object()
    ...
    def get(self, key, defaultValue) -> int:
        ...
        while self.t[hashedIndex] is not None and self.t[hashedIndex] != self.dl:
            if self.t[hashedIndex][0] != key:
                hashedIndex = hashedIndex + 1 % len(self.t)
                continue
            else:
                return self.t[hashedIndex][1]  # ass defval later'''
        return defaultValue
    ...
    def items(self):
        ...
        for i in range(len(self.t)):
            if self.t[i] is not None and self.t[i] != self.dl:
                final.append(self.t[i])
        return final
